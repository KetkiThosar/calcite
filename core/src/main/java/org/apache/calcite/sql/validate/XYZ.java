...
public class XYZ {
	public static RelDataType createTypeFromProjection(RelDataType type, List<String> columnNameList,
		...
		// differences,
		// the resulting type will use those from type. These are presumably
		// more
		// canonical.
	 ...
	 * @param table
	 *            the target table or null if it is not a RelOptTable instance
	 */
	public static RelDataTypeField getTargetField(RelDataType rowType, RelDataTypeFactory typeFactory, SqlIdentifier id,
		...
		final List<Pair<RelDataTypeField, List<String>>> entries = ((CustomColumnResolvingTable) t)
				.resolveColumn(rowType, typeFactory, id.names);
		switch (entries.size()) {
		case 1:
	 ...
	 *
	 * @param leftType
	 *            Row type of left input to join
	 * @param rightType
	 *            Row type of right input to join
	 * @param joinType
	 *            Type of join
	 * @param typeFactory
	 *            Type factory
	 * @param fieldNameList
	 *            List of names of fields; if null, field names are inherited
	 *            and made unique
	 * @param systemFieldList
	 *            List of system fields that will be prefixed to output row
	 *            type; typically empty but must not be null
	 * @return join type
	 */
	public static RelDataType deriveJoinRowType(RelDataType leftType, RelDataType rightType, JoinRelType joinType,
			RelDataTypeFactory typeFactory, List<String> fieldNameList, List<RelDataTypeField> systemFieldList) {
	 ...
	 *
	 * <p>
	 * The resulting row type consists of the system fields (if any), followed
	 * by the fields of the left type, followed by the fields of the right type.
	 * The field name list, if present, overrides the original names of the
	 * fields.
	 *
	 * @param typeFactory
	 *            Type factory
	 * @param leftType
	 *            Type of left input to join
	 * @param rightType
	 *            Type of right input to join, or null for semi-join
	 * @param fieldNameList
	 *            If not null, overrides the original names of the fields
	 * @param systemFieldList
	 *            List of system fields that will be prefixed to output row
	 *            type; typically empty but must not be null
	...
	public static RelDataType createJoinType(RelDataTypeFactory typeFactory, RelDataType leftType,
			RelDataType rightType, List<String> fieldNameList, List<RelDataTypeField> systemFieldList) {
		assert (fieldNameList == null) || (fieldNameList
				.size() == (systemFieldList.size() + leftType.getFieldCount() + rightType.getFieldCount()));
		...
		final Set<String> uniqueNameList = typeFactory.getTypeSystem().isSchemaCaseSensitive() ? new HashSet<>()
	 ...
	 *
	 * @param sourceRowType
	 *            The source upon which to ordinate the bit set.
	 * @param indexToField
	...
	public static ImmutableBitSet getOrdinalBitSet(RelDataType sourceRowType,
		...
		ImmutableBitSet source = ImmutableBitSet
				.of(Lists.transform(sourceRowType.getFieldList(), RelDataTypeField::getIndex));
	 ...
	 *
	 * @param sourceFields
	 *            The source of column names that determine indexes
	 * @param targetFields
